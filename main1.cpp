...
void lagrangian(double xi, int n, double** & ders) {
    ders = new double*[n + 1];
    for (int m = 0; m < n + 1; m++)
        ders[m] = new double[3];
    ders[0][0] = -xi * (1.0 - xi) / 2;
    ders[0][1] = (1.0 - xi * xi);
    ders[0][2] = xi * (1.0 + xi) / 2;
    if (n == 1) {
        ders[1][0] = xi - .5;
        ders[1][1] = -2 * xi;
        ders[1][2] = xi + .5;
...
public:
    T operator()(T x, T y, T t, T tbar, T c0) {
        return pow(c0, 2) * f(tbar) * Heaviside(c0 * (t - tbar) - sqrt(x * x + y * y)) / 2 / Pi / c0 /
...
int main() {
    ...
    int elements_x = 25, elements_y = 25;
    ...
    int dof_x = 2 * elements_x + 1, dof_y = 2 * elements_y + 1;
    int dof = dof_x * dof_y;
        ...
        for (int ii_y = 0; ii_y < elements_y; ii_y++) {
            ...
            for (int jj_x = 0; jj_x < gaussian_points; jj_x++) {
                for (int jj_y = 0; jj_y < gaussian_points; jj_y++) {
                    ...
                    lagrangian(gaussian[jj_x], 1, ders_x);
                    lagrangian(gaussian[jj_y], 1, ders_y);
                    VectorXd Nxi = VectorXd::Zero(p_x + 1), Nxi_xi = VectorXd::Zero(p_x + 1), Neta = VectorXd::Zero(p_y + 1), Neta_eta = VectorXd::Zero(
                                       p_y + 1);
                    for (int kk_x = 0; kk_x < p_x + 1; kk_x++) {
                        Nxi(kk_x) = ders_x[0][kk_x];
                        Nxi_xi(kk_x) = ders_x[1][kk_x] / J_x;
                    }
                    for (int kk_y = 0; kk_y < p_y + 1; kk_y++) {
                        Neta(kk_y) = ders_y[0][kk_y];
                        Neta_eta(kk_y) = ders_y[1][kk_y] / J_y;
                    }
                    for (int k = 0; k < 2; k++)
                        delete ders_x[k];
                    delete[] ders_x;
                    ...
                    for (int iii_x = 0; iii_x < (p_x + 1) * (p_y + 1); iii_x++) {
                        for (int iii_y = 0; iii_y < (p_x + 1) * (p_y + 1); iii_y++) {
                            M(((2 * (i_x - p_x)) * dof_y) + (i_y - p_y) * 2 + iii_x / (p_y + 1) * dof_y + iii_x % (p_y + 1),
                              ((2 * (i_x - p_x)) * dof_y) + (i_y - p_y) * 2 + iii_y / (p_y + 1) * dof_y + iii_y % (p_y + 1)) += M_ele(iii_x, iii_y);
                        }
                    }
                }
            }
...
*/
        ...
        for (int ii_y = 0; ii_y < elements_y; ii_y++) {
            ...
            for (int jj_x = 0; jj_x < gaussian_points; jj_x++) {
                for (int jj_y = 0; jj_y < gaussian_points; jj_y++) {
                    ...
                    for (int iii_x = 0; iii_x < (p_x + 1) * (p_y + 1); iii_x++) {
                        for (int iii_y = 0; iii_y < (p_x + 1) * (p_y + 1); iii_y++) {
                            K(((2 * (i_x - p_x)) * dof_y) + (i_y - p_y) * 2 + iii_x / (p_y + 1) * dof_y + iii_x % (p_y + 1),
                              ((2 * (i_x - p_x)) * dof_y) + (i_y - p_y) * 2 + iii_y / (p_y + 1) * dof_y + iii_y % (p_y + 1)) += K_ele(iii_x, iii_y);
                        }
                    }
                }
            }
    ...
    */
    cout<<t<<endl;
    ofstream outf("modified.txt");
    ofstream outf1("exact.txt");
    outf << "x" << " " << "u" << endl;
    for (int i = 0; i < dof_x; i++)
        outf << setprecision(10) << "(" << i *h / 2 << ","
             << d_current(i * dof_y) << ")" << endl;
    for (int i = 0; i < 101; i++)
        outf1 << setprecision(10) << "(" << i * 1.0 / 100 << ","
              << trapezoidal(0.0, 7.9999999999999, integrand<double>(), 8000000, 0.0,  i * 1.0 / 100, c0) << ")" << endl;
    return 0;
}
